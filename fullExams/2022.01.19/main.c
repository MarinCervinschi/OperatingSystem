...
int main(int argc, char **argv) {
    ...
    char *FCreato;   /* variabile per nome file da creare da parte dei processi
                        figli primi della coppia */
    ...
    int nroTotale; /* variabile per tenere traccia del numero di blocchi
                      presenti nei file */
    int nro; /* variabile per tenere traccia del numero di blocchi leti dalla
                propria meta' del file */
    ...
    /* allocazione memoria dinamica per pipe_ps. NOTA BENE: servono un numero di
     * pipe che e' la meta' del numero di figli! */
    ...
    for (i = 0; i < 2 * N; i++) {
        ...
        if (pid == 0) {
            /* codice del figlio: in caso di errore torniamo 0 che non e' un
             * valore accettabile (per quanto risulta dalla specifica della
             * parte shell) */
            if (i < N) /* siamo nel codice dei figli primi della coppia */
            {
                /* stampa di debugging */
                printf(
                    "DEBUG-PRIMO DELLA COPPIA-Figlio di indice %d e pid %d "
                    "associato al file %s\n",
                    i, getpid(), argv[i + 1]);
                ...
                if (fd < 0) {
                    ...
                    exit(0); /* in caso di errore, decidiamo di tornare 0 che
                                non e' un valore accettabile */
                }
                ...
                for (j = 0; j < nro; j++) {
                    read(fd, b, C);
                    /* ogni blocco letto dal PRIMO processo della coppia deve
                     * essere inviato al processo SECONDO della coppia */
            ...
            } else /* siamo nel codice dei figli secondi della coppia */
            {
                /* stampa di debugging */
                printf(
                    "DEBUG-SECONDO DELLA COPPIA-Figlio di indice %d e pid %d "
                    "associato al file %s\n",
                    i, getpid(), argv[i - N + 1]);
                /* i figli secondi della coppia devono creare il file
                 * specificato */
                FCreato = (char *)malloc(strlen(argv[i - N + 1]) + 11); 
                            /* bisogna allocare una stringa lunga come il nome del
                            file associato + il carattere '.' + i caratteri
                            della parola mescolato (9) + il terminatore di
                            stringa: ATTENZIONE ALL'INDICE PER INDIVIDUARE IL
                            FILE */
                if (FCreato == NULL) {
                ...
                /* ogni figlio deve aprire il suo file associato: siamo nei
                 * figli secondi della coppia e quindi attenzione all'indice */
                ...
                for (j = 0; j < nro; j++) {
                    read(fd, b, C);
                    /* ogni blocco letto dal processo SECONDO della coppia,
                     * bisogna scriverlo sul file */
                    write(fcreato, b, C);
                    /* dobbiamo a questo punto aspettare il blocco dal processo
                     * PRIMO della coppia: attenzione all'indice */
                    nr = read(pipe_ps[i - N][0], b, C);
                    if (nr != C) {
                        printf("Errore in lettura da pipe %d\n", i - N);
                        exit(0);
                    }
                    /* ogni blocco ricevuto dal processo PRIMO della coppia,
                     * bisogna scriverlo sul file */
                    write(fcreato, b, C);
                }
            }
            exit(nro); /* torniamo il numero di blocchi letti (supposto <= di
                          255) */
        }
    }
