...
int main (int argc, char **argv) {   
   ...
   int pidFiglio, status, ritorno;      /* per wait padre */
	...
	if (pid == 0) {   
        ...
        /* -------------cancello la chiusura del lato di lettura della pipe */
        ...
        /* il contenuto del file e' tale che in mess ci saranno 4 caratteri e il terminatore di linea */
		while (read(fd, mess, MSGSIZE)) {   
    		/* il padre ha concordato con il figlio che gli mandera' solo stringhe e quindi dobbiamosostituire il terminatore di linea con il terminatore di stringa */
    		mess[MSGSIZE-1] = '\0'; 
    		write(piped[1], mess, MSGSIZE);	/* il figlio che e' il produttore comunica al padre lalinea letta dal file dopo averla trasformata in stringa (per il protocollo dicomunicazione stabilito con il padre */
    ...
    /* questo ciclo avra' termine appena il figlio terminera' dato che la read senza piu' scrittore tornera' 0! */
	while (read(piped[0], mess, MSGSIZE)) { 	
        /* dato che il figlio gli ha inviato delle stringhe, il padre le puo' scrivere direttamentecon una printf */
        printf("%d: %s\n", j, mess);
        j++;
	}
