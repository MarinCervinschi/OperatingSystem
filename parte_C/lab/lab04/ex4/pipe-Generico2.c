...
int main(int argc, char **argv) {
    if (argc != 2) {
        printf("ERROR ----> Numero dei parametri errato %d: ci vuole un singolo parametro\n", argc);
        exit(1);
    }
    ...
    if (pipe(piped) < 0) {
        printf("ERROR -----> creazione pipe\n");
        exit(2);
    }

    int pid;
    if ((pid = fork()) < 0) {
        printf("ERROR -----> creazione figlio\n");
        exit(3);
    }
